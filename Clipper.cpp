...
public:
    ...
    void setGeometry(osg::Geometry* geometry, const Clipper::IndexList& protectedPoints);
    ...
    unsigned int clipTriangles(        const osg::BoundingBox &bbox)
    {
        std::vector<Triangle *> tri_ptr;
        for(TriangleSet::iterator itr=_triangleSet.begin();
            itr!=_triangleSet.end();
            ++itr)
        {

            if(testTriangleClip(bbox,const_cast<Triangle*>(itr->get()))>0){
               tri_ptr.push_back(const_cast<Triangle*>(itr->get()));
            }
        }
        for(int i=0; i<tri_ptr.size(); i++)
         removeTriangle(tri_ptr[i]);
    ...
    unsigned int  testTriangleClip(const osg::BoundingBox &bbox,Triangle* triangle)
    {
        unsigned int  result = 0;
        if (!bbox.contains(triangle->_p1->_vertex))
        {
...
//            OSG_NOTICE<<"testTriangle("<<triangle<<") _p1==NULL"<<std::endl;
        ...
        if (!bbox.contains(triangle->_p3->_vertex))
        {
//            OSG_NOTICE<<"testTriangle("<<triangle<<") _p1==NULL"<<std::endl;
            ++result;
        }
...
void EdgeCollapse::setGeometry(osg::Geometry* geometry, const Clipper::IndexList& protectedPoints)
{
    _geometry = geometry;
    ...
    for(Clipper::IndexList::const_iterator pitr=protectedPoints.begin();
...
void Clipper::simplify(osg::Geometry& geometry)
{
...
void Clipper::simplify(osg::Geometry& geometry, const IndexList& protectedPoints)
{
...
//               ec._triangleSet.size() < targetNumTriangles  &&
    ...
    {
        OSG_INFO<<std::endl<<"Clipper, in = "<<numOriginalPrimitives<<"\tout = "<<ec._triangleSet.size()<<"\terror="<<(*ec._edgeSet.begin())->getErrorMetric()<<"\tvs "<<getMaximumError()<<std::endl<<std::endl;
