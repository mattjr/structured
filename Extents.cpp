...
bool MyDataSet::computeOptimumLevel(Source* source, int maxLevel, int& level)
{
...
*/
/*int MyDataSet::computeMaximumLevel(int maxNumLevels)
{
    int maxLevel = 0;
...
*/
/*bool MyDataSet::computeOptimumTileSystemDimensions(int& C1, int& R1)
{
    C1 = 1;
    R1 = 1;
                                                      ...
                                                      unsigned int maxNumLevels)
{
    ...
    destinationGraph->_tileX = currentX;
    destinationGraph->_tileY = currentY;
    destinationGraph->_dataSet = (DataSet*)this;
...
bool MyDataSet::prepareForDestinationGraphCreation()
{
...
void MyDataSet::computeDestinationGraphFromSources(unsigned int numLevels)
{
    ...
    {
        log(osg::NOTICE,"Old MyDataSet::createDestinationGraph() selected");
...
void MyDataSet::assignDestinationCoordinateSystem()
{
    if (getDestinationCoordinateSystem().empty() && !getConvertFromGeographicToGeocentric())
    {
        for(CompositeSource::source_iterator itr(_sourceGraph.get());itr.valid();++itr)
        {
            ...
            log(osg::NOTICE,"MyDataSet::assignDestinationCoordinateSystem() : assigning first source file as the destination coordinate system");
            break;
        }
    }
}
...
void MyDataSet::assignIntermediateCoordinateSystem()
{
    ...
    if (!_intermediateCoordinateSystem)
    {
        ...
        log(osg::INFO, "new MyDataSet::createDestination()");
...
bool MyDataSet::requiresReprojection()
{
    assignIntermediateCoordinateSystem();
...
void MyDataSet::reprojectSourcesAndGenerateOverviews()
{
...
void MyDataSet::updateSourcesForDestinationGraphNeeds()
{
    ...
    // a MIPSpro compiler error "The member "vpb::MyDataSet::_sourceGraph" is inaccessible."
...
void MyDataSet::populateDestinationGraphFromSources()
{
    if (!_destinationGraph || !_sourceGraph) return;

    log(osg::NOTICE, "started MyDataSet::populateDestinationGraphFromSources)");
    ...
    log(osg::NOTICE, "completed MyDataSet::populateDestinationGraphFromSources)");
}
...
void MyDataSet::_writeNodeFile(osg::Node& node,const std::string& filename)
{
...
void MyDataSet::_writeImageFile(osg::Image& image,const std::string& filename)
{
    if (getDisableWrites()) return;
...
public:
        ...
        _writeHint(osg::Image::STORE_INLINE)
    {
        ...
        if (MyDataSet->getDestinationTileExtension()==".osg" ||
            MyDataSet->getDestinationTileExtension()==".osgt" ||
            MyDataSet->getDestinationTileExtension()==".osgx") _writeHint = osg::Image::EXTERNAL_FILE;
    }

    vpb::MyDataSet*           _MyDataSet;
    ...
    void writeLayer(osgTerrain::Layer* layer)
    {
        ...
        if (imageLayer)
        {
            ...
            if (image)
            {
                _MyDataSet->log(osg::NOTICE,"Writing out image layer %s, _directory=%s ",image->getFileName().c_str(),_directory.c_str());
                if (needToWriteOutImage(image)) _MyDataSet->_writeImageFile(*image,_directory+image->getFileName());
            }
            return;
        }
        ...
        if (switchLayer)
        {
            ...
            {
                _MyDataSet->log(osg::NOTICE,"Writing out switch layer child: %s, %s ",switchLayer->getSetName(i).c_str(),switchLayer->getFileName(i).c_str());
        ...
        if (compositeLayer)
        {
            ...
            {
                _MyDataSet->log(osg::NOTICE,"Writing out composite layer child: %s",switchLayer->getFileName(i).c_str());
    ...
    void apply(osg::StateSet& stateset)
    {
        for(unsigned int i=0;i<stateset.getTextureAttributeList().size();++i)
        {
            ...
            {
                _MyDataSet->_writeImageFile(*image,_directory+image->getFileName());
            }
        }
    }
};

void MyDataSet::_writeNodeFileAndImages(osg::Node& node,const std::string& filename)
{
    if (getDisableWrites()) return;
...
void MyDataSet::createDestination(unsigned int numLevels)
{
    log(osg::NOTICE, "started MyDataSet::createDestination(%u)",numLevels);
    ...
    log(osg::NOTICE, "completed MyDataSet::createDestination(%u)",numLevels);

}

osg::Node* MyDataSet::decorateWithTerrain(osg::Node* subgraph)
{
    ...
    log(osg::NOTICE, "called MyDataSet::decorateWithTerrain()");

    return terrain;
}

osg::Node* MyDataSet::decorateWithCoordinateSystemNode(osg::Node* subgraph)
{
...
osg::Node* MyDataSet::decorateWithMultiTextureControl(osg::Node* subgraph)
{
...
void MyDataSet::addSource(Source* source, unsigned int revisionNumber)
{
    if (!source) return;
...
bool MyDataSet::addModel(Source::Type type, osg::Node* model, unsigned int revisionNumber)
{
...
bool MyDataSet::addLayer(Source::Type type, osgTerrain::Layer* layer, unsigned layerNum, unsigned int revisionNumber)
{
...
bool MyDataSet::addTerrain(osgTerrain::TerrainTile* terrainTile, unsigned int revisionNumber)
{
...
bool MyDataSet::addTerrain(osgTerrain::TerrainTile* terrainTile)
{
...
bool MyDataSet::addPatchedTerrain(osgTerrain::TerrainTile* previous_terrain, osgTerrain::TerrainTile* new_terrain)
{
...
osgTerrain::TerrainTile* MyDataSet::createTerrainRepresentation()
{
...
bool MyDataSet::createTileMap(unsigned int level, TilePairMap& tilepairMap)
{
    osg::CoordinateSystemNode* cs = _intermediateCoordinateSystem.get();
...
bool MyDataSet::generateTasks(TaskManager* taskManager)
{
...
void MyDataSet::selectAppropriateSplitLevels()
{
...
std::string MyDataSet::getTaskName(unsigned int level, unsigned int X, unsigned int Y) const
{
    if (level==0 && X==0 && Y==0)
    {
...
std::string MyDataSet::getSubtileName(unsigned int level, unsigned int X, unsigned int Y) const
{
    std::ostringstream os;
...
bool MyDataSet::generateTasksImplementation(TaskManager* taskManager)
{
    log(osg::NOTICE,"MyDataSet::generateTasks_new");
...
const std::string MyDataSet::getDatabaseRevisionBaseFileName(unsigned int level, unsigned int x, unsigned y) const
{
    std::string baseName;
...
std::string MyDataSet::checkBuildValidity()
{
