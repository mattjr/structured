...
int main(int argc, char** argv)
{
    ...
    Ysize=height*_tileRows;
    {
        ...
        rawI=im_open("tmp","p");
        if(im_black(rawI,Xsize,Ysize,4)){
            fprintf(stderr,"Can't create color image\n");
            return -1;
        }

        IMAGE *diskFile=im_open("subtile.v","w");
        im_copy(rawI,diskFile);
        im_close(rawI);
        rawI=diskFile;
    ...
    /*{
        vips::VImage tmp;
        tmp.initdesc(width*_tileColumns,height*_tileRows,3,vips::VImage::FMTUCHAR,vips::VImage::NOCODING,vips::VImage::sRGB,1.0,1.0,0,0);
        tmp.black(width*_tileColumns,height*_tileRows,3);
        tmp.write("subtile.v");
        if(untex)
            tmp.write("subtile_untex.v");
        Xsize=tmp.Xsize();
        Ysize=tmp.Ysize();

    }*/
    {
        // vips::VImage raw("subtile.v","w");
        // vips::VImage raw_untex("subtile_untex.v","rw");
            ...
            if(nogfx){
                char tmp[1024];
                {
                    ...
                    //    cout << tmp <<endl;
                        ...
                        REGION *reg2=      im_region_create(tmpI);
                       REGION * regOutput = im_region_create(rawI);
                        if(regOutput == NULL){
                        ...
                        r2.top=0;
                        r2.width=tmpI->Xsize;
                        r2.height=tmpI->Ysize;
                        if(im_prepare(regOutput,&r)){
                            fprintf(stderr,"Prepare fail\n");
                            exit(-1);
                        }

                     /*   if(im_prepare_to(reg2,regOutput,&r2,r.left,r.top)){
                            fprintf(stderr,"Prepare fail\n");
                            exit(-1);
                        }*/
                        if(im_prepare(reg2,&r2)){
                            fprintf(stderr,"Prepare fail\n");
                            exit(-1);
                        }
                        if(!im_isfile(rawI)){
                            fprintf(stderr,"INCORE!!!\n");
                        }
                        im_region_copy(reg2,regOutput,&r2,r.left,r.top);
                        double vm, rss;
                        process_mem_usage(vm, rss);
                        cout << "VM: " << get_size_string(vm) << "; RSS: " << get_size_string(rss) << endl;
