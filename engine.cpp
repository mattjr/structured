...
vector<Tex_Pose_Data> load_tex_pose_file( const string &file_name )
{
    vector<Tex_Pose_Data> poses;
...
int main( int argc, char *argv[ ] )
{
    ...
    for(int i=0; i< (int)tasks.size(); i++){
        ...
        if(!osgDB::fileExists(texfilename)){
            IplImage *img=cvLoadImage((base_dir+"/img/"+tasks[i].file_name).c_str(),-1);
            if(img){
                cacheImage(img,texfilename,cachedtexdir[0].second);
                cvReleaseImage(&img);
            }
            else
...
#pragma omp atomic
    ...
    vrip_cells=calc_cells(tasks,vrip_img_per_cell);
                ...
                {
                    {
                        ...
                        for(int i=0; i <(int)cells.size(); i++){
                            ...
                            for(int k=0; k < (int)cells[i].imagesMargin.size(); k++){
                                const Tex_Pose_Data *pose=(&tasks[cells[i].imagesMargin[k]]);
                                if(pose && pose->valid){
