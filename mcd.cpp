...
void addChunk(void*p,int size,char*fun,const char*file,int line)
{
#ifdef MCD_FASTFREE
...
void *MCD_malloc(int size,char*fun,const char*file,int line)
{
#ifdef MCD_FASTFREE
...
int MCD_fscanf(FILE *stream,const char *fmt,const char*fun,char*file,int line,...)
{
	int retval;
	va_list argptr;
