		     ...
		     vpColVector &weights)
{
  ...
  int n_all_data = all_residues.size();
...
vpRobust::scale(vpRobustEstimatorType method, vpColVector &x)
{
  ...
  for(int i=0; i<n; i++)
  {
...
#if VP_DEBUG_MODE == 3
    {
      ...
      std::cout << "Sum chi = " << chiTmp*sqrt(sig_prev) << std::endl;
  ...
  sigma2 = Sum_chi*sqrt(sig_prev)/((n-p)*Expectation);
...
vpRobust::constrainedChiTukey(double x)
{
  ...
  {
    //sct = (sqrt(s*a-x)*sqrt(s*a+x)*sqrt(x))/(s*sqrt(sqrt(a*sqrt(s))));
    sct = (sqrt(s*a)*x-s*sqrt(s*a)-x*sqrt(x))*(sqrt(s*a)*x+s*sqrt(s*a)-x*sqrt(x))/s*sqrt(sqrt(sqrt(s)))/sqrt(sqrt(a));
  }
  else
    sct = -1/s;
...
vpRobust::constrainedChiCauchy(double x)
{
  ...
  sct = -1*(sqrt(x)*b)/(s*(sqrt(s*b)+sqrt(x)));
...
vpRobust::psiTukey(double sig, vpColVector &x)
{
  ...
  for(int i=0; i<n_data; i++)
  {
    ...
    {
      w[i] = sqrt(1-sqrt(xi_sig/cst_const));
      //w[i] = sqrt(1-sqrt(x[i]/sig/4.7));
    }
    else
    {
...
vpRobust::psiMcLure(double sig, vpColVector &r)
{
  ...
  for(int i=0; i<n_data; i++)
  {
    w[i] = 1/(sqrt(1+sqrt(r[i]/sig)));
    //w[i] = 2*mad/sqrt((mad+r[i]*r[i]));//odobez
