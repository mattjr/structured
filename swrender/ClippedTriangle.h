...
public:
    ClippedTriangle(const osg::Vec2& a, const osg::Vec2& b,  const osg::Vec2& c)
	{
		m_numVertices = 3;
	...
	{
        osg::Vec2 * v  = m_vertexBuffers[m_activeVertexBuffer];
		m_activeVertexBuffer ^= 1;
        osg::Vec2 * v2 = m_vertexBuffers[m_activeVertexBuffer];
		...
		for (uint k=0; k<m_numVertices; k++)
		{
			...
			if ( dy1in + dy2in == 1 ) // not both in/out
			{
                ...
                v2[p++] = osg::Vec2(v[k].x() + dy1*(dx/dy), offset);
			}
	...
	inline void clipVerticalPlane(float offset, float clipdirection ) 
	{
		...
		for (uint k=0; k<m_numVertices; k++)
		{
			...
			if ( dx1in + dx2in == 1 ) // not both in/out
			{
				float dx = v[k+1].x() - v[k].x();
				float dy = v[k+1].y() - v[k].y();
                v2[p++] = osg::Vec2(offset, v[k].y() + dx1*(dy/dx));
			}
	...
	void computeAreaCentroid()
	{
		...
		if (m_area==0) {
            m_centroid = osg::Vec2(0,0);
		} else {
            m_centroid = osg::Vec2(centroidx/(6*m_area), centroidy/(6*m_area));
		}
	}
...
private:
    osg::Vec2 m_verticesA[7+1];
    osg::Vec2 m_verticesB[7+1];
    osg::Vec2 * m_vertexBuffers[2];
	uint    m_numVertices;
