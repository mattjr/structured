...
void GLCamera::setupGL(const point &scene_center, float scene_size,bool ortho) const
{
	GLint V[4];
	glGetIntegerv(GL_VIEWPORT, V);
	...
	if(ortho){
	   left = scene_center[0] - scene_size;
	   right = scene_center[0] + scene_size;
	   bottom = scene_center[1] - scene_size;
	   top = scene_center[1] + scene_size;
	}else{
