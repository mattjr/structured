...
static void write_ply_ascii(TriMesh *mesh, FILE *f,
			    bool write_norm, bool float_color,const char *comments);
static void write_ply_binary(TriMesh *mesh, FILE *f,
			     bool need_swap, bool write_norm, bool float_color,const char *comments);
...
void TriMesh::write(const char *filename,const char *comments)
{
	if (!filename || *filename == '\0')
		return;
	...
	switch (filetype) {
		case PLY_ASCII:
		  write_ply_ascii(this, f, write_norm, float_color,comments);
			break;
		case PLY_BINARY_BE:
			write_ply_binary(this, f,
					 we_are_little_endian(), write_norm, float_color,comments);
			break;
		case PLY_BINARY_LE:
			write_ply_binary(this, f,
					 !we_are_little_endian(), write_norm, float_color,comments);
...
static void write_ply_ascii(TriMesh *mesh, FILE *f, bool write_norm,
			    bool float_color,const char *comments)
{
	if (write_norm)
		mesh->need_normals();
	...
	write_ply_header(mesh, f, "ascii", write_grid, write_tstrips,
			 write_norm, float_color,comments);
