...
#ifndef __GENERIC_ATTRIBUTES_IO__
#define __GENERIC_ATTRIBUTES_IO__
...
namespace OGF {
    ...
    template <class T> inline bool get_serializable_attributes(
    ...
    template <class T> inline void serialize_read_attributes(
        std::istream& in, T* item, std::vector<SerializedAttribute<T> >& attributes
    ) {
    ...
    template <class T> inline void serialize_write_attributes(
        std::ostream& out, T* item, std::vector<SerializedAttribute<T> >& attributes
    ) {
        for(unsigned int i=0; i<attributes.size(); i++) {
