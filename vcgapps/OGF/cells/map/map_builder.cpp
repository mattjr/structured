...
//_________________________________________________________
    ...
    void MapBuilder::end_surface() {
        delete all_facet_keys ; all_facet_keys = nil ;
    ...
    void MapBuilder::add_vertex(unsigned int id, const Point3d& p) {
        transition(surface, surface) ;
        add_vertex_internal(id, p) ;
    }

    void MapBuilder::add_vertex(const Point3d& p) {
        transition(surface, surface) ;
        add_vertex_internal(p) ;
    }

    void MapBuilder::add_tex_vertex(const Point2d& p) {
        add_tex_vertex(tex_vertex_.size(), p) ;
    }

    void MapBuilder::add_tex_vertex(unsigned int id, const Point2d& p) {
        transition(surface, surface) ;
    ...
    void MapBuilder::add_vertex_internal(unsigned int id, const Point3d& p) {
    ...
    void MapBuilder::add_vertex_internal(const Point3d& p) {
    ...
    void MapBuilder::end_facet() {
        ogf_assert(
            facet_vertex_.size() == facet_tex_vertex_.size()
        ) ;
        int nb_vertices = facet_vertex_.size() ;
        
        if(nb_vertices < 3) {
            if(!quiet_) {
                Logger::err("MapBuilder") << "Facet with less than 3 vertices"
                                          << std::endl ;
            }
            return ;
        }
                    ...
                    if(facet_vertex_[i] == facet_vertex_[j]) {
                        if(!quiet_) {
                            Logger::err("MapBuilder") 
                                << "Duplicated vertex in facet, ignoring facet"
                                << std::endl ;
                        }
                        return ;
                    }
