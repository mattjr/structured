...
// functions when applicable (to copy the attributes)
...
namespace OGF {
    ...
    void MapEditor::set_target(Map* target) {
        ...
        if(tex_vertex_normal_.is_bound()) {
            tex_vertex_normal_.unbind() ;
        }
        tex_vertex_normal_.bind(target) ;
...
//__________ CGAL interface __________________________________
    ...
    MapEditor::Vertex* MapEditor::create_center_vertex(Facet* f) {
        ...
        // Interpolate attributes
        {
            ...
            target()->vertex_attribute_manager()->interpolate_record_begin(v) ;
            Map::Halfedge* h = f->halfedge() ;
            do {
                target()->vertex_attribute_manager()->interpolate_record_add(w, h->vertex()) ;
                h = h->next() ;
            } while(h != f->halfedge()) ;
            target()->vertex_attribute_manager()->interpolate_record_end() ;
        }

        Point3d p = Geom::facet_barycenter(f) ;
        Point2d u = Geom::facet_barycenter2d(f) ;
    ...
    MapEditor::Vertex* MapEditor::split_edge(
        ...
        Point3d p = Geom::edge_barycenter(h)  ;
        Point2d u = Geom::edge_barycenter2d(h) ;
        ...
        target()->vertex_attribute_manager()->interpolate_record_2(v, 0.5, ov1, 0.5, ov2) ;
...
//_____________________ additional functions __________________
    ...
    bool MapEditor::glue(
        ...
        Point3d new_p0 = Geom::barycenter(
            h0-> vertex()-> point(), h1-> opposite()-> vertex()-> point() 
        ) ;
        
        Point3d new_p1 = Geom::barycenter(
...
/*
    Map::TexVertex* MapEditor::new_tex_vertex(TexVertex* lhs) {
        TexVertex* result = new_tex_vertex() ;
        result->set_tex_coord(lhs->tex_coord()) ;
        tex_vertex_normal_[result] = tex_vertex_normal_[lhs] ;
        return result ;
    }
*/
    ...
    void MapEditor::merge_tex_vertices(const Vertex* v_in) {
        ...
        Point2d u = Geom::vertex_barycenter2d(v) ;
        set_tex_vertex_on_orbit(v-> halfedge(), new_tex_vertex()) ;
        v->halfedge()->set_tex_coord(u) ;
    }
    ...
    void MapEditor::merge_tex_vertices(
        ...
        Point2d u1 = Geom::vertex_barycenter2d(v1) ;
        Point2d u2 = Geom::vertex_barycenter2d(v2) ;
        Point2d u  = Geom::barycenter(u1, u2) ;
    ...
    void MapEditor::split_surface(SplitMode mode) {
        ...
        case split_faces:
            split_all_facets() ;
            break ;
        case split_catmull_clark:
            split_all_edges(1) ;
            split_all_facets(2) ;
            catmull_clark_update_geometry() ;
            remove_edges(0,2) ;
            break ;
        case split_catmull_clark_backup_geometry:
            split_all_edges(1) ;
            split_all_facets(2) ;
            {
                MapVertexAttribute<Point3d> geometry(target(), "geometry") ;
                FOR_EACH_VERTEX(Map, target(), it) {
                    geometry[it] = it->point() ;
                }
            }
            catmull_clark_update_geometry() ;
            remove_edges(0,2) ;
...
//_________________________________________________________
    ...
    void MapEditor::insert_edges_in_facet(Facet* f, int id) {
        ...
        ogf_assert(vertex_id_[start-> next()-> vertex()] == id) ;
...
//_________________________________________________________
    ...
    // Yes, I know, it will be good to get rid of those Point3d
    // and Vector3d classes, and replace everything with a
    // GLSL-like vec3 class with operators overloads (as in Geex) ...

    static inline void add_points(Point3d& p, const Point3d& q) {
        p.set_x(p.x() + q.x()) ;
        p.set_y(p.y() + q.y()) ;
        p.set_z(p.z() + q.z()) ;
    }

    static inline void mul_point(Point3d& p, double s) {
        p.set_x(s * p.x()) ;
        p.set_y(s * p.y()) ;
        p.set_z(s * p.z()) ;
    }

    void MapEditor::catmull_clark_update_geometry() {

        // c.f. http://en.wikipedia.org/wiki/Catmull-Clark_subdivision_surface

        // Fix edge points geometry
        FOR_EACH_VERTEX(Map, target(), it) {
            if(vertex_id_[it] == 1) {
                Point3d p(0,0,0) ;
                int N = 0 ;
                ...
                do {
                    ...
                    if(vertex_id_[v] == 0 || vertex_id_[v] == 2) {
                        add_points(p, v->point()) ;
                        N++ ;
                    }
                    h = h->next_around_vertex() ;
                } while(h != it->halfedge()) ;
                mul_point(p, 1.0/double(N)) ;
                it->set_point(p) ;
            }
        }

        // Fix original points geometry
        FOR_EACH_VERTEX(Map, target(), it) {
            if(vertex_id_[it] == 0) {
                Point3d P = it->point() ;
                Point3d R(0,0,0) ;
                ...
                do {
                    ...
                    if(vertex_id_[v] == 1) {
                        NR++ ;
                        add_points(R, v->point()) ;
                    } else if(vertex_id_[v] == 2) {
                        NF++ ;
                        add_points(F, v->point()) ;
                    }
                    h = h->next_around_vertex() ;
                } while(h != it->halfedge()) ;
                mul_point(R, 1.0/double(NR)) ;
                mul_point(F, 1.0/double(NF)) ;
                it->set_point(
                    Point3d(
                        (F.x() + 2.0 * R.x() + double(NR-3)*P.x()) / double(NR),
                        (F.y() + 2.0 * R.y() + double(NR-3)*P.y()) / double(NR),
                        (F.z() + 2.0 * R.z() + double(NR-3)*P.z()) / double(NR)
                    )
                ) ;
            }
        }
    ...
    void MapEditor::inside_out(bool reorient_tex_vertex_normal) {
        ...
        if(reorient_tex_vertex_normal && MapTexVertexNormal::is_defined(target())) { 
            MapTexVertexNormal tex_vertex_normal(target()) ;
            Attribute<Map::TexVertex, bool> visited(
                target()->tex_vertex_attribute_manager()
            ) ;
            { FOR_EACH_HALFEDGE(Map, target(), it) {
                Map::TexVertex* tv = it->tex_vertex() ;
                if(!visited[tv]) {
                    visited[tv] = true ;
                    tex_vertex_normal[tv] = -1.0 * tex_vertex_normal[tv] ;
                }
            }}
        }
    ...
    void MapEditor::inside_out(MapComponent* comp, bool reorient_tex_vertex_normal) {
