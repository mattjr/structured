...
//_______ Geometry ________________________________________________________
    ...
    namespace Geom {
        ...
        void translate_component(MapComponent* comp, const Vector3d& v) {
            FOR_EACH_VERTEX(MapComponent, comp, it) {
                Point3d new_p = it->point() + v ;
                it->set_point(new_p) ;
            }
        }
        
        void translate_component2d(MapComponent* comp, const Vector2d& v) {
            FOR_EACH_VERTEX(MapComponent, comp, it) {
                Point2d new_p = it->halfedge()->tex_coord() + v ;
