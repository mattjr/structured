...
// [Cedric] porting for WIN32
#ifdef WIN32
#define drand48 rand
#endif

// [Bruno] This one controls swapping according to endian
static int do_swap = 0 ;

// [Bruno] Added this one to check machine endian
static int machine_is_little_endian() {
    int x = 0x01020304 ;
    char* p = (char *)(&x) ;
    ...
    abort() ;
    // If we get there, we really have a weird machine.
    // This may happen if standard ints are 64 bits...
}
  ...
  while (words) {
    ...
    if (equal_strings (words[0], "format")) {
      ...
      if (equal_strings (words[1], "ascii"))
          ...
          do_swap = machine_is_little_endian() ; // [Bruno]
