 ...
 */
    METHODDEF(void) my_error_exit (j_common_ptr cinfo) {
		...
		Logger::err("JPEG Loader") << buffer << std::endl ;
    ...
    Image* ImageSerializer_jpeg::serialize_read(const std::string& filename_in) {
		Logger::out("JPEG Loader") << "Loading " << filename_in << std::endl ;
        Image* result = nil ;
        ...
        if ((infile = fopen(filename, "rb")) == NULL) {
            Logger::err("ImageSerializer_jpeg") << "could not open file:" << filename_in << std::endl ;
            return nil ;
        }
        ...
        Logger::out("JPEG loader") << "JPEG image: " << cinfo.output_width << " x " << cinfo.output_height
        ...
        while (cinfo.output_scanline < cinfo.output_height) {
			...
			// just incremented it !!
            Memory::copy(result->pixel_base(0, cinfo.output_scanline-1), buffer[0], row_stride) ;
        }
