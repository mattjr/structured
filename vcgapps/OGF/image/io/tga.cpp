 ...
 *
 * $Id: tga.cpp 439 2003-01-31 00:03:25Z levy $
...
static int
std_fread(
   RLEstate *rleInfo, void *buf, int datasize, int nelems, FILE *fp
)
{
  if (verbose > 1) {
...
gliReadTGA(FILE *fp, char *name)
{
  ...
  int start, end, dir;
  int i, j, k;
  int pelbytes, wbytes, cmappelbytes = 0 ;
  GLenum format;
  int components;
  RLEstate rleRec;
  RLEstate *rleInfo;
  ...
  int (*myfread)(RLEstate *rleInfo, void*, int, int, FILE*);
  ...
  switch(cmappelbytes) {
  case 3:
      genericImage->cmapFormat = GL_BGR_EXT;  
      break ;
  case 4:
      genericImage->cmapFormat = GL_BGRA_EXT;  
      break ;
  case 0:
      genericImage->cmapFormat = 0 ;        
      break ;
  default:
      genericImage->cmapFormat = 0 ;        
      if(verbose) printf(
          "TGA: unknown cmap entry size: %d bytes\n",cmappelbytes
      ) ;
      break ;
  }
...
void gliFreeImage(gliGenericImage* image) {
    free(image->cmap) ;  
    free(image->pixels) ;
    free(image) ;
}
