...
#ifndef __OGF_MATH_GEOMETRY_BOX__
#define __OGF_MATH_GEOMETRY_BOX__
...
namespace OGF {
    ...
    public:
        ...
        GenericPoint2d<T> center() const {
            return GenericPoint2d<T>((x_max() + x_min())/2, (y_max() + y_min())/2) ;
        }
        double radius() const {
            return 0.5 * ::sqrt( ogf_sqr(x_max()-x_min()) + ogf_sqr(y_max()-y_min()) ) ;
        }
        void add_point(const GenericPoint2d<T>& p) {
            if(!initialized_) {
                x_min_ = p.x() ;
                y_min_ = p.y() ;
                x_max_ = p.x() ;
        ...
        void add_box(const GenericBox2d<T>& b) {
            if(b.initialized()) {
                add_point(GenericPoint2d<T>(b.x_min(), b.y_min())) ;
                add_point(GenericPoint2d<T>(b.x_max(), b.y_max())) ;
            }
        }
...
//_________________________________________________________________________
    ...
    public:
        ...
        GenericPoint3d<T> center() const {
            return GenericPoint3d<T>(
                0.5*(x_max() + x_min()), 
                0.5*(y_max() + y_min()),
                0.5*(z_max() + z_min())
            ) ;
        ...
        void add_point(const GenericPoint3d<T>& p) {
            if(!initialized_) {
                x_min_ = p.x() ;
                y_min_ = p.y() ;
                z_min_ = p.z() ;
        ...
        void add_box(const GenericBox3d<T>& b) {
            if(b.initialized()) {
                add_point(GenericPoint3d<T>(b.x_min(), b.y_min(), b.z_min())) ;
                add_point(GenericPoint3d<T>(b.x_max(), b.y_max(), b.z_max())) ;
            }
        }
