/* cnapps.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int cnapps_(n, kev, np, shift, v, ldv, h__, ldh, resid, q, 
	ldq, workl, workd)
integer *n, *kev, *np;
complex *shift, *v;
integer *ldv;
...
complex *workl, *workd;
{
    ...
    /* Local variables */
    static integer iend;
    static real unfl, ovfl, c__;
    static complex f, g;
    static integer i__, j;
    static complex r__, s, t;
    extern /* Subroutine */ int cscal_();
    static complex sigma;
    extern /* Subroutine */ int cgemv_(), ccopy_(), caxpy_(), cmout_(), 
	    cvout_();
    static real t0, t1;
    extern /* Subroutine */ int ivout_();
    static complex h11, h21;
    extern doublereal slapy2_();
    static integer jj;
    extern /* Subroutine */ int slabad_();
    extern doublereal clanhs_(), slamch_();
    extern /* Subroutine */ int clacpy_();
    static integer istart, kplusp, msglvl;
    static real smlnum;
    extern /* Subroutine */ int clartg_(), claset_(), second_();
...
/*     %---------------------------------------------------% */
    ...
    for (j = 1; j <= i__1; ++j) {
	i__2 = j + 1 + j * h_dim1;
	if (h__[i__2].r < (float)0. || r_imag(&h__[j + 1 + j * h_dim1]) != (
		float)0.) {
	    i__2 = j + 1 + j * h_dim1;
	    i__3 = j + 1 + j * h_dim1;
	    r__2 = h__[i__3].r;
