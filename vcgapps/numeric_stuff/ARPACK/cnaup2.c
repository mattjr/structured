/* cnaup2.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int cnaup2_(ido, bmat, n, which, nev, np, tol, resid, mode, 
...
ftnlen which_len;
{
    ...
    extern /* Subroutine */ int cnaitr_(), cngets_(), cnapps_(), csortc_();
...
/*        %---------------------------------------------------------% */
    ...
    for (j = 1; j <= i__1; ++j) {
	i__2 = j;
	if (bounds[i__2].r == (float)0. && bounds[i__2].i == (float)0.) {
	    --(*np);
	    ++(*nev);
	}
/* L30: */
