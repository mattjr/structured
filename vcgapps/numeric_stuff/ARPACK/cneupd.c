/* cneupd.f -- translated by f2c (version 19991025).
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int cneupd_(rvec, howmny, select, d__, z__, ldz, sigma, 
	workev, bmat, n, which, nev, tol, resid, ncv, v, ldv, iparam, ipntr, 
	workd, workl, lworkl, rwork, info, howmny_len, bmat_len, which_len)
logical *rvec;
char *howmny;
logical *select;
complex *d__, *z__;
integer *ldz;
complex *sigma, *workev;
char *bmat;
integer *n;
char *which;
integer *nev;
...
ftnlen which_len;
{
    ...
    static integer ritz, j, k, iheig;
    extern /* Subroutine */ int cscal_();
    static integer ihbds;
    extern /* Complex */ VOID cdotc_();
    extern /* Subroutine */ int cgeru_();
    static real conds;
    static logical reord;
    extern /* Subroutine */ int ccopy_(), ctrmm_();
    static integer nconv;
    static real rtemp;
    extern /* Subroutine */ int cmout_();
    static complex rnorm;
    extern /* Subroutine */ int cvout_(), ivout_(), cgeqr2_();
    extern doublereal scnrm2_();
    extern /* Subroutine */ int cunm2r_();
    extern doublereal slapy2_();
    static integer ih, jj, iq, np;
    static complex vl[1];
    extern doublereal slamch_();
    static integer wr, bounds, invsub, iuptri, msglvl, outncv, numcnv, ishift;
    extern /* Subroutine */ int clacpy_(), clahqr_(), cngets_(), claset_(), 
	    ctrsen_(), ctrevc_(), csscal_();
    static integer ibd, ldh, ldq;
    static real sep;
    static integer irz;
...
/*     %------------------------------------% */
    ...
    i__1 = ih + 2;
    workl[i__1].r = (float)0., workl[i__1].i = (float)0.;
...
/*        %---------------------------------------------------% */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = bounds + j - 1;
	    workl[i__2].r = (real) j, workl[i__2].i = (float)0.;
	    select[j] = FALSE_;
/* L10: */
	}
...
upd: Ritz estimates of the untransformed Ritz values.", (ftnlen)56);
    } else if (msglvl > 1) {
...
/*        %------------------------------------------------% */
	...
	for (j = 1; j <= i__1; ++j) {
	    i__2 = iheig + j - 1;
	    if (workl[i__2].r != (float)0. || workl[i__2].i != (float)0.) {
		i__2 = j;
