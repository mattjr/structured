/* slaqrb.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int slaqrb_(wantt, n, ilo, ihi, h__, ldh, wr, wi, z__, info)
logical *wantt;
integer *n, *ilo, *ihi;
real *h__;
integer *ldh;
...
integer *info;
{
    ...
    /* Local variables */
    static real h43h34, unfl, ovfl, work[1];
    ...
    extern /* Subroutine */ int slarfg_();
    extern doublereal slanhs_();
    static real smlnum, h33s, h44s;
...
/*     %---------------------------------------------------------------% */
	...
	for (i__ = 1; i__ <= i__1; ++i__) {
	    h__[i1 + i__ + 1 + i__ * h_dim1] = (float)0.;
/* L8: */
	}
    } else {
	i__1 = *ihi - *ilo - 1;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    h__[*ilo + i__ + 1 + (*ilo + i__ - 1) * h_dim1] = (float)0.;
/* L9: */
	}
    }
...
/* Computing MIN */
		...
		h__[k + 1 + (k - 1) * h_dim1] = (float)0.;
		if (k < i__ - 1) {
		    h__[k + 2 + (k - 1) * h_dim1] = (float)0.;
		}
	    } else if (m > l) {
