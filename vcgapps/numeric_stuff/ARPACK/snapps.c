/* snapps.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int snapps_(n, kev, np, shiftr, shifti, v, ldv, h__, ldh, 
	resid, q, ldq, workl, workd)
integer *n, *kev, *np;
real *shiftr, *shifti, *v;
integer *ldv;
real *h__;
integer *ldh;
real *resid, *q;
...
real *workl, *workd;
{
    ...
    /* Local variables */
    static integer iend;
    static real unfl, ovfl, c__, f, g;
    static integer i__, j;
    static real r__, s, t, u[3];
    static logical cconj;
    extern /* Subroutine */ int sscal_(), slarf_(), sgemv_(), scopy_();
    static real t0, t1;
    extern /* Subroutine */ int saxpy_(), ivout_(), smout_(), svout_();
    static real h11, h12, h21, h22, h32;
    ...
    static real smlnum;
    extern /* Subroutine */ int slacpy_(), slarfg_(), slaset_(), slartg_();
    extern doublereal slanhs_();
...
/*           %-----------------------------------------% */
	    ...
	    cconj = FALSE_;
	    goto L110;
	} else if (jj < *np && dabs(sigmai) > (float)0.) {
...
/*        %------------------------------------------------------% */
	...
	if (istart + 1 == iend && dabs(sigmai) > (float)0.) {
	    goto L100;
	}

	h11 = h__[istart + istart * h_dim1];
	h21 = h__[istart + 1 + istart * h_dim1];
	if (dabs(sigmai) <= (float)0.) {
...
/*                 %-------------------------------------------% */
		    ...
		    h__[i__ + 1 + (i__ - 1) * h_dim1] = (float)0.;
		}
...
/*              %-----------------------------------------------------% */
		...
		if (i__ > istart) {
		    ...
		    h__[i__ + 1 + (i__ - 1) * h_dim1] = (float)0.;
		    if (i__ < iend - 1) {
			h__[i__ + 2 + (i__ - 1) * h_dim1] = (float)0.;
		    }
		}
		u[0] = (float)1.;
...
/*     %-------------------------------------------------% */
    ...
    if (h__[*kev + 1 + *kev * h_dim1] > (float)0.) {
