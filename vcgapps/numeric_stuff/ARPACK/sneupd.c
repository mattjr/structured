/* sneupd.f -- translated by f2c (version 19991025).
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int sneupd_(rvec, howmny, select, dr, di, z__, ldz, sigmar, 
	sigmai, workev, bmat, n, which, nev, tol, resid, ncv, v, ldv, iparam, 
	ipntr, workd, workl, lworkl, info, howmny_len, bmat_len, which_len)
logical *rvec;
char *howmny;
logical *select;
real *dr, *di, *z__;
integer *ldz;
real *sigmar, *sigmai, *workev;
char *bmat;
integer *n;
char *which;
integer *nev;
...
ftnlen which_len;
{
    ...
    static integer j, k, ihbds, iconj;
    extern /* Subroutine */ int sscal_();
    static real conds;
    static logical reord;
    extern /* Subroutine */ int sgemv_();
    static integer nconv, iwork[1];
    static real rnorm;
    extern /* Subroutine */ int scopy_();
    static integer ritzi;
    extern /* Subroutine */ int strmm_();
    static integer ritzr;
    extern /* Subroutine */ int smout_(), ivout_(), svout_(), sgeqr2_();
    extern doublereal slapy2_();
    static integer ih, jj;
    extern /* Subroutine */ int sorm2r_();
    static integer iheigi, np;
    static real vl[1]	/* was [1][1] */;
    static integer iheigr, bounds, invsub, iuptri, msglvl, outncv, ishift, 
	    numcnv;
    extern /* Subroutine */ int slacpy_(), slahqr_(), slaset_(), strevc_(), 
	    strsen_();
    extern doublereal slamch_();
    extern /* Subroutine */ int sngets_();
...
pd: Real part of Ritz values after calling _NGETS.", (ftnlen)54);
	    svout_(&debug_1.logfil, ncv, &workl[iri], &debug_1.ndigit, "_neu\
pd: Imag part of Ritz values after calling _NGETS.", (ftnlen)54);
...
/*           %---------------------------------------------------% */
	    ...
	    if (workl[invsub + (j - 1) * ldq + j - 1] < (float)0.) {
