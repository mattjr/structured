/* sngets.f -- translated by f2c (version 19991025).
...
/* Subroutine */ int sngets_(ishift, which, kev, np, ritzr, ritzi, bounds, 
	shiftr, shifti, which_len)
integer *ishift;
char *which;
integer *kev, *np;
real *ritzr, *ritzi, *bounds, *shiftr, *shifti;
ftnlen which_len;
{
    ...
    /* Local variables */
    static real t0, t1;
    extern /* Subroutine */ int ivout_(), svout_(), second_();
    static integer msglvl;
    extern /* Subroutine */ int ssortc_();
...
/*     %-------------------------------------------------------% */
    ...
    if (ritzr[*np + 1] - ritzr[*np] == (float)0. && ritzi[*np + 1] + ritzi[*
	    np] == (float)0.) {
	--(*np);
	++(*kev);
    }
