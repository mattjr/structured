/* sseupd.f -- translated by f2c (version 19991025).
...
/* ----------------------------------------------------------------------- */
/* Subroutine */ int sseupd_(rvec, howmny, select, d__, z__, ldz, sigma, bmat,
	 n, which, nev, tol, resid, ncv, v, ldv, iparam, ipntr, workd, workl, 
	lworkl, info, howmny_len, bmat_len, which_len)
logical *rvec;
char *howmny;
logical *select;
real *d__, *z__;
...
ftnlen which_len;
{
    ...
    static real rnorm;
    extern /* Subroutine */ int scopy_(), ivout_(), svout_();
    static real bnorm2;
    extern /* Subroutine */ int sgeqr2_();
    ...
    extern /* Subroutine */ int slacpy_(), ssesrt_(), ssteqr_(), ssortr_(), 
	    ssgets_();
    static integer ibd, ihb, ihd, ldh, ldq, irz, leftptr, rghtptr;
...
/*        %-------------------------------------------------------------% */
	...
	if (s_cmp(type__, "SHIFTI", (ftnlen)6, (ftnlen)6) == 0) {
	    i__1 = *ncv;
	    for (k = 1; k <= i__1; ++k) {
		workl[ihd + k - 1] = (float)1. / workl[ihd + k - 1] + *sigma;
/* L40: */
	    }
...
/* L50: */
	    ...
	    i__1 = *ncv;
	    for (k = 1; k <= i__1; ++k) {
		workl[ihd + k - 1] = *sigma * (workl[ihd + k - 1] + (float)1.)
			 / (workl[ihd + k - 1] - (float)1.);
/* L60: */
	    }
	}
...
/*        %-----------------------------------------------------% */
	...
	i__1 = *ncv - 1;
	for (j = 1; j <= i__1; ++j) {
	    workl[ihb + j - 1] = (float)0.;
/* L65: */
	}
	workl[ihb + *ncv - 1] = (float)1.;
...
/* L90: */
		...
		workl[ihb + k - 1] = (r__1 = workl[ihb + k - 1] / workl[iw + 
			k - 1] * (workl[iw + k - 1] - (float)1.), dabs(r__1));
/* L100: */
	    }
