...
/* -----------------------------------------------------------------------------
 * CHOLMOD/Core Module.  Version 1.0.  Copyright (C) 2005, Univ. of Florida.
...
)
{
    ...
    PRINT1 (("really new need %ld current %ld\n", (long) need,
	    (long) (Lp [Lnext [j]] - Lp [j]))) ;
    ASSERT (need >= 1 && need <= n-j) ;

    if (Lp [Lnext [j]] - Lp [j] >= (Int) need)
    {
	/* no need to reallocate the column, it's already big enough */
	PRINT1 (("colrealloc: quick return %ld %ld\n",
	    (long) (Lp [Lnext [j]] - Lp [j]), (long) need)) ;
	return (TRUE) ;
