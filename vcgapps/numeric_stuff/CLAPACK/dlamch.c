...
/* 
 * [Bruno]
 * Got problems with g++ in optimize mode, dlamch enters an infinite loop,
 * replaced with statically computed values from the non-optimized version.
 */

#ifndef USE_ORIGINAL_DLAMCH

#include <ctype.h>
#include <assert.h> 
#include <string.h> 

static char* keys = "ESBPNRMULO";

static double values[] = {
1.11022302462515654042363e-16,
2.22507385850720138309023e-308,
2,
...
2.22507385850720138309023e-308,
1024,
1.79769313486231570814527e+308,
};

doublereal dlamch_(char *cmach) {
    char key = toupper(*cmach) ;
    int nb_keys = strlen(keys) ;
    int i ;
    for(i=0; i<nb_keys; i++) {
        if(key == keys[i]) {
            return values[i] ;
