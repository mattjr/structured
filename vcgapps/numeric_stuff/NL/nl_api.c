...
static double nlGetResidual() {
    double result = 0 ;
    double* y = NL_NEW_ARRAY(double, nlCurrentContext->n) ;
    NLuint i ;
    double cur ;
    nlCurrentContext->matrix_vector_prod(nlCurrentContext->x,y);    
    for(i=0; i<nlCurrentContext->n; i++) {
        cur = nlCurrentContext->b[i] - y[i] ;
        result += cur*cur ;
    }
    NL_DELETE_ARRAY(y) ;
    result = sqrt(result) ;
...
void nlGetDoublev(NLenum pname, NLdouble* params) {
    ...
    case NL_MATRIX_STORE: {
        *params = (NLdouble)(nlCurrentContext->matrix_store) ;        
    } break ;
    case NL_RESIDUAL: {
        *params = nlGetResidual() ;
    } break ;
    default: {
        nl_assert_not_reached ;
    } break ;
...
void nlGetIntergerv(NLenum pname, NLint* params) {
    ...
    case NL_MATRIX_STORE: {
        *params = (NLint)(nlCurrentContext->matrix_store) ;        
    }
    default: {
        nl_assert_not_reached ;
    } break ;
...
void nlBeginMatrix() {
    ...
    if(nlCurrentContext->solver == NL_TAUCS_EXT) {
        storage = (storage & ~NL_MATRIX_STORE_ROWS) ;
        storage = (storage |  NL_MATRIX_STORE_COLUMNS) ;       
    }

    if(nlCurrentContext->matrix_store != NL_MATRIX_STORE_DEFAULT) {
        storage = nlCurrentContext->matrix_store ;
    } else {
        nlCurrentContext->matrix_store = storage ;
    }
