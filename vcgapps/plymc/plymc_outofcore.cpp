...
const char *diced_dir;
inline bool ProgressBarCallback(const int pos,const int total ,unsigned long long starttick, const char *str ) {
    formatBar(str,starttick,pos,total);
    return true;}

inline bool NullCallback(const int pos,const int total ,unsigned long long starttick, const char *str ) {
    return true;}
int main(int argc, char *argv[])
{
  ...
  case 'D' : p.VerboseLevel=1; p.SliceNum=atoi(argv[i]+2);printf("Enabling Debug Volume saving of %i slices (VerboseLevel=1)\n",p.SliceNum);break;
    ...
    case 'i': processing_step=atoi(argv[i]+2);printf("Running Step %d\n",processing_step);break;
		default : {printf("Error unable to parse option '%s'\n",argv[i]); exit(0);}
    }
    ++i;
  }

  if(processing_step <0 ||processing_step >2){
      fprintf(stderr,"Must run processing step 0, 1, or 2\n");
      exit(-1);
  }
  Matrix44f Identity;
  ...
  case 0:  pmc.ProcessCells(NullCallback);
    break;
  case 1:  pmc.ProcessNormalize(NullCallback);
    break;
  case 2:  pmc.ProcessMC(NullCallback);
    break;
