        ...
        for(int i=0;i <passesFlatRemap; i++){
            ...
            for( map<int,vector<ply::tri_t> >::iterator itr=vertexData[ORIG_MAPPING]._img2tri.begin(); itr!=vertexData[ORIG_MAPPING]._img2tri.end(); itr++){
                for(int t=0; t< (int)itr->second.size(); t++){
                    ...
                    if(i ==1 ){
                        if(process_tri(itr->second[t],use_vert,vertexData[ORIG_MAPPING]._texCoord,blending)){
                        ...
                        //viSamp.setCurrentFace(vtxCount, positions, normals);*/
                        if(process_tri_new(itr->second[t],vertexData[REMAPPED_MAPPING]._vertices,vertexData[REMAPPED_MAPPING]._texCoord,blending,sizeX,sizeY,viSamp,sData)){
                    ...
                    if(i==1){
                       ...
                       /* int vtxCount=3;
                        ...
                        for (uint k = 0; k < vtxCount; k++)
                        {
                            ...
                            texcoords[k].set(vertexData[ORIG_MAPPING]._texCoord[pos]->at(itr->second[t].idx[k]).x(),
                                             1.0-vertexData[ORIG_MAPPING]._texCoord[pos]->at(itr->second[t].idx[k]).y());
                            //newVerts->at(itr->second[t].idx[k]).z());
