...
void write_file(PlyObject &o,PlyObject &B);
...
Face * find_match(Face *f, PlyObject &A, PlyObject &B);
...
double sqdist(Vertex *v1,Vertex *v2){
  double dx = v1->x - v2->x;
  double dy = v1->y - v2->y;
  double dz = v1->z - v2->z;
...
find_match(Face *f, PlyObject &A, PlyObject &B) {
  ...
  //double dist= sqdist(v,A.vlist[f->verts[0]]);
  //  fprintf(stderr,"d=%f\n",dist);
  for (fl = v->faces;fl != NULL; fl = fl->next) {
	...
	newface->nverts=3;
	newface->verts=new int[3];
	newface->verts[0]=A.nverts+A.vlist[f->verts[0]]->closest->index; 
	newface->verts[1]=f->verts[0];
	newface->verts[2]=f->verts[1];
	return newface;
	//      }
    }
  }
...
subtract_vertices(PlyObject &A, PlyObject &B)
{
  ...
  for (i = 0; i < A.nfaces; i++) {
    newface=find_match(A.flist[i], A, B);
    if (newface) {
...
void
write_file(PlyObject &o,PlyObject &B)
{
  int i,j,k;
  PlyFile *ply;
  int num_elems;
  ...
  fprintf(stderr,"Num new faces %d\n",newfaces.size());
  ply = ply_write (stdout, o.nelems, o.elist, PLY_ASCII);//o.file_type);

  /* count the vertices that are in the hash table */

  vert_count = 0;
  for (i = 0; i < o.nverts; i++) {
    if ((o.vlist[i]->nfaces > 0) ) {
      // renumber vertices.
  ...
  for (i = 0; i < B.nverts; i++) {
    if ((B.vlist[i]->nfaces > 0) ) {
      // renumber vertices.
      B.vlist[i]->index = vert_count;
      vert_count++;
    }
  }

  /* count the faces that do not have matches */
  face_count = o.nfaces+B.nfaces;
 ...
 for (i = 0; i < B.nverts; i++) {
    if (B.vlist[i]->nfaces > 0)  {
      ply_put_element (ply, (void *) B.vlist[i]);
    }
  }
  ...
  for (i = 0; i < B.nfaces; i++) {
    ...
    // Reset the numbering scheme
    for (j = 0; j < B.flist[i]->nverts; j++) {
      B.flist[i]->verts[j] = B.flist[i]->vptrs[j]->index;
    }
    // write out tri
    ply_put_element (ply, (void *) B.flist[i]);
  }

  
  for(i=0; i < (int)newfaces.size(); i++){
    ply_put_element (ply, (void *) newfaces[i]);
  }
